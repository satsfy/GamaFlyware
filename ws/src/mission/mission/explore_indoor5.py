...
class DroneControl(RosConfig):
    ...
    def publish_position_setpoint_smoothed(
        self, x: float, y: float, z: float, heading: float = None, smooth_factor: float = 0.93
    ):
        """
...
class DetectaQRCodes(NoFase3):
    ...
    def navage_para_proxima_base(self):
        x, y, z = self.coordenadas.get_posicao_base_atual()
        dx, dy, dz, yaw = self.trajectory.get_drone_x_y_z_heading(should_fail=False)
        general_logger.info(
        ...
        self.publish_position_setpoint_smoothed(x, y, z, smooth_factor=0.93)
        ...
        if self.trajectory.is_in_position_2d(x, y, 1.0):
            for i in range(3):
                ...
                if not self.trajectory.is_in_position_2d(x, y, 1.0):
                    general_logger.info(f"esta_em_cima_da_base_odometria fail on self.trajectory.is_in_position_2d attempt {i}")
                    return False
            return True
