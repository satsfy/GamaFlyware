...
class Coordenadas:
    def __init__(self):
        ...
        self.bases = [
            # [-L, -20.0, 16.0, 1.57079, 0, 0],
            # [-L, -20.0, 16.0, 0.0, 0, 0],
            # [-L, -20.0, 16.0, -1.57079, 0, 0],
            # [-L, -20.0, 16.0, 3.14, 0, 0],
            [
                L - PADDING_HORIZONTAL,
                -20.0 - PADDING_HORIZONTAL,
                16.0 + PADDING_HEIGHT,
                0.0,
                0,
                0,
        ...
        general_logger.warn(f"Coordenadas initialized with bases: {self.bases}")
    ...
    def get_posicao_base_atual(self) -> tuple[float, float, float, float]:
        if (
        ...
        return (
            ...
            float(self.new_bases[self.indice_base_atual][3]),
        )
...
class RosConfig(Node):
    ...
    def __init__(self, name: str = "ros_config") -> None:
        ...
        self.trajectory = Trajectory(logger=general_logger)
...
class DroneControl(RosConfig):
    ...
    def publish_position_setpoint_smoothed(
        ...
        if hasattr(self, "last_published_setpoint") and self.last_published_setpoint:
            curr_pos = np.array(self.last_published_setpoint.position)
            curr_yaw = self.last_published_setpoint.yaw
        else:
        ...
        self.last_setpoint = msg
        # if you also publish/store last_published_setpoint elsewhere, update it there
...
class NoFase3(DroneControl):
    ...
    def esta_no_ar_estavel(self) -> bool:
        ...
        if self.trajectory.is_stable(
            ...
            try:
                if self.trajectory.is_in_position_3d(spx, spy, spz, 0.3):
                    return True
                else:
                    general_logger.info(f"self.trajectory.is_in_position_3d failed")
            except Exception as e:
                general_logger.error(e)
                general_logger.info(f"self.trajectory.is_in_position_3d ERROR!!")
        else:
            general_logger.info(f"self.trajectory.is_stable failed")
        return False
    ...
    def navegue_para_base(self):
        x, y, _, _ = self.coordenadas.get_posicao_base_atual()
        self.publish_position_setpoint(x, y, TAKEOFF_HEIGHT)
...
class DetectaQRCodes(NoFase3):
    ...
    def navage_para_proxima_base(self):
        x, y, z, yaw = self.coordenadas.get_posicao_base_atual()
        dx, dy, dz, dyaw = self.trajectory.get_drone_x_y_z_heading(should_fail=False)
        general_logger.info(
            f"Posicao base   [{round(x,3)}, {round(y,3)}, {round(z,3)}]\n"
        )
        general_logger.info(
            f"Posicao drone  [{round(dx,3)}, {round(dy,3)}, {round(dz,3)}]\n"
        )
        self.publish_position_setpoint_smoothed(
            x, y, z, heading=yaw, smooth_factor=0.985, yaw_smooth_factor=0.994
        )

    def esta_em_cima_da_base_odometria(self) -> bool:
        x, y, _, _ = self.coordenadas.get_posicao_base_atual()
        if self.trajectory.is_in_position_2d(x, y, 3.0):
            for i in range(3):
                self.custom_sleep(0.5)
                if not self.trajectory.is_in_position_2d(x, y, 3.0):
                    general_logger.info(
                        f"esta_em_cima_da_base_odometria fail on self.trajectory.is_in_position_2d attempt {i}"
                    )
                    return False
            return True
        general_logger.info("esta_em_cima_da_base_odometria fail on general ")
        return False
    ...
    def afaste_drone_para_encontrar_base(self):
        ...
        _, _, base_z, _ = self.coordenadas.get_posicao_base_atual()
    ...
    def aproximar_do_chao_para_ler_qrcode(self):
        ...
        x, y, base_z, _ = self.coordenadas.get_posicao_base_atual()
